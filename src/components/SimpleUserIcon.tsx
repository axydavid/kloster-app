...
const SimpleUserIcon: React.FC<SimpleUserIconProps> = ({ user, guests = 0, showRemoveButton = false, onRemove, onClick, isTakeAway = false, size = 'normal' }) => {
    ...
    <div className="relative group" onClick={onClick}>
      {user.id === 'guest' ? (
        ...
        >
          ...
          {isTakeAway && (
            <div className={`absolute ${size === 'small' ? '-top-0.5 -right-0.5 w-2 h-2' : '-top-1 -right-1 w-3 h-3'} bg-yellow-400 rounded-full border border-white`}></div>
          )}
        </div>
      )}
      {guests > 0 && (
